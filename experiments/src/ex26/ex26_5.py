...
OUTPUT_FILE = "/experiments/ex26/ex26_5.txt"
...
def evalOne(n_estimators, max_depth):
    Y = []
    P = []
...
#         test_upper = [float(testStationList[i]) for i in range(0, len(testStationList)) if i >= (len(testStationList) / 2.0)]
        ...
        model = RandomForestClassifier(random_state=42, n_estimators=n_estimators, max_depth=max_depth, n_jobs=-1)
...
for n_estimators in range(10,500,10):
    for max_depth in range(2,15):
        f1, accuracy = evalOne(n_estimators, max_depth)
        log("n:" + str(n_estimators) + ",depth:" + str(max_depth) + ": " + str(f1) + ",accuracy: " + str(accuracy))
