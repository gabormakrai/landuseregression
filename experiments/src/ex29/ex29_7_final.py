...
OUTPUT_FILE = "/experiments/ex29/ex29_7_final.csv"
OUTPUT_LOG_FILE = "/experiments/ex29/ex29_7_final.txt"
...
output.write("prediction,observation,model,station\n")
...
for i in range(0, len(all_observations)):
    ...
    pred_TWA = all_pred_TW[i]
    pred_lower = all_pred_lower[i]
    pred_upper = all_pred_upper[i]
    ...
    output.write(str(obs) + "," + str(pred_lower) + ",TW_lower," + str(location) + "\n")
    output.write(str(obs) + "," + str(pred_upper) + ",TW_upper," + str(location) + "\n")
