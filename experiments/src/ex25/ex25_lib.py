...
def loadEx25Data(fileName):
    ...
    with open(fileName) as infile:
        ...
        for line in infile:                
            ...
            #location,timestamp,tag,value
            location = splittedLine[0]
            timestamp = splittedLine[1]
            tag = splittedLine[2]
            prediction = float(splittedLine[3])
            
            data[tag][location][timestamp] = prediction
    return data
...
def getXYFromData(data, columns_X, column_Y):
    Y = data[column_Y]
    X = []
