...
OUTPUT_FILE = "/experiments/ex23/file.csv"
...
output.write("train_station,test_station,RMSE\n")
for trainStation in locations:
    for testStation in locations:
        rmse = evalTrainStationTestStation(float(trainStation), float(testStation))
        output.write(trainStation + "," + testStation + "," + str(rmse) + "\n")
        output.flush()
