...
for depth in range(1,60):
    for estimators in [40]:
        ...
        for iteration in range(0, 5):
            ...
            model = RandomForestRegressor(max_depth=depth, n_estimators = estimators, n_jobs = -1, random_state=42)
            model.fit(trainX, trainY)
            prediction = model.predict(testX)
            rmse = rmseEval(testY, prediction)[1]
            rmses.append(rmse)
        print("depth: " + str(depth) + ": avg_rmse:" + str(numpy.mean(rmses)) + " " + str(rmses))    