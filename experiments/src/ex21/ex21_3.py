...
for leaf in [10*i for i in range(30,60)]:
    for estimators in [40]:
        #print("leaf:" + str(leaf) + ",estimators:" + str(estimators))
        rmses = []
        for iteration in range(0, 5):
            ...
            model = RandomForestRegressor(max_leaf_nodes= leaf, n_estimators = estimators, n_jobs = -1, random_state=42)
            model.fit(trainX, trainY)
            prediction = model.predict(testX)
            rmse = rmseEval(testY, prediction)[1]
            rmses.append(rmse)
        print("leaf: " + str(leaf) + ": avg_rmse:" + str(numpy.mean(rmses)) + " " + str(rmses))    