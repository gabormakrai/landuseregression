...
OUTPUT_DATA_FILE = "/experiments/ex1/ex1_rf6.csv"

parametersList = []

for max_leaf in range(2,15000,10):
    for n in [20,40,60]:
        parametersList.append({"n_estimators": n, "max_leaf": max_leaf})
...
def evalOne(parameters):
    ...
    for location in locations:
        ...
        if "depth" in parameters:
            model = RandomForestRegressor(max_depth = parameters["depth"], random_state=42, n_estimators=parameters["n_estimators"], n_jobs=3)
        elif "leaf" in parameters:
            model = RandomForestRegressor(min_samples_leaf = parameters["leaf"], random_state=42, n_estimators=parameters["n_estimators"], n_jobs=3)
        elif "max_leaf" in parameters:
            model = RandomForestRegressor(max_leaf_nodes = parameters["max_leaf"], random_state=42, n_estimators=parameters["n_estimators"], n_jobs=3)
