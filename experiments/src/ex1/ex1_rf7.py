...
OUTPUT_DATA_FILE = "/experiments/ex1/ex1_rf7.csv"

parametersList = []

for n in range(5,500):
    for max_leaf in [5000,6000,7000]:
...
def evalOne(parameters):
    ...
    for location in locations:
        ...
        if "depth" in parameters:
            model = RandomForestRegressor(max_depth = parameters["depth"], random_state=42, n_estimators=parameters["n_estimators"], n_jobs=2)
        elif "leaf" in parameters:
            model = RandomForestRegressor(min_samples_leaf = parameters["leaf"], random_state=42, n_estimators=parameters["n_estimators"], n_jobs=2)
        elif "max_leaf" in parameters:
            model = RandomForestRegressor(max_leaf_nodes = parameters["max_leaf"], random_state=42, n_estimators=parameters["n_estimators"], n_jobs=2)
