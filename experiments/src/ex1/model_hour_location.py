...
from crossvalidation import crossValidationLocation
...
for year in years:
    ...
    outputDir = "/media/sf_lur/experiments/ex2/"
    ...
    for evalName in evalFunctionNames:
        ...
        doErrorBar(outputDir + evalName + "_" + str(year) + ".png", title, xAxis, yAxis, names, means, stddevs)
  
    for result in results:
        x = result[1]["scatter"]["test"]
        y = result[1]["scatter"]["prediction"]
        doScatterDiagram(outputDir + "scatter_" + result[0] + "_" + str(year) + ".png", "Model output (" + str(year) + ") - " + result[0], "Observation (ug/m3)", "Prediction (ug/m3)", x, y) 
